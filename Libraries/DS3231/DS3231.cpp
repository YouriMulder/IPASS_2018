...
void DS3231::ResetCurrentCenturyBit() {
	setI2CBusCurrentAddress();
...
void DS3231::update() {
	if(getCurrentCenturyBit() && !newCentury) {
		...
		ResetCurrentCenturyBit();
	}
}

void DS3231::printTest(bool condition, const char testCaseMessage[]) {
...
void DS3231::test() {
	hwlib::cout << "DS3131 test\n";
	// Testing seconds
	uint8_t testSeconds = 15;
	setCurrentSeconds(testSeconds);
	printTest(getCurrentSeconds() == testSeconds, "seconds register");

	// Testing minutes
	uint8_t testMinutes = 58;
	setCurrentMinutes(testMinutes);
	printTest(getCurrentMinutes() == testMinutes, "minutes register");

	// Testing Hours
	uint8_t testHours = 12;
	setCurrentHours(testHours);
	printTest(getCurrentHours() == testHours, "hours register");
	...
	hwlib::wait_ms(1500);
	printTest(getCurrentDate() == testDate + 1, "hours overflow test, date should increase");

	// timestamp test
	timestamp testTimestamp;

	setCurrentSeconds(0);
	setCurrentMinutes(21);
	setCurrentHours(17);
	setCurrentDay(5);
	setCurrentDate(1);
	setCurrentMonth(6);
	...
	getCurrentTimestamp(testTimestamp);

	hwlib::cout << "Timestamp test start\n";
	printTest(testTimestamp.getSeconds() 	== 0, "timestamp seconds");
	printTest(testTimestamp.getMinutes() 	== 21, "timestamp minutes");
	printTest(testTimestamp.getHours() 		== 17, "timestamp hours");
	printTest(testTimestamp.getDay() 		== 5, "timestamp day");
	printTest(testTimestamp.getDate() 		== 1, "timestamp date");
	printTest(testTimestamp.getMonth() 		== 6, "timestamp month");
	printTest(testTimestamp.getYear() 		== 18, "timestamp year");
	hwlib::cout << "Timestamp test end\n";
