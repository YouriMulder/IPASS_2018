...
public:
	...
	/// @param UID[] The array you want the UID to get stored into.
	...
	/// @brief Wait for a card te be presented and print the UID.
	/// @details
	/// This method waits for a card to be presented.
	/// After a card has been detected the UID will be printed to the terminal.
	/// @warning This function blocks everything!
	virtual void printUID() = 0;

public:
	/// @brief check whether two UIDs are equal
	/// @param UID1[] the first UID you want to check.
	/// @param UID2[] the seconds UID you want to check.
	...
	/// @param UID[] the array where the UID including the BCC is store.
	/// @details
...
public:
	...
	/// Testing requires a rfid 4 UID sized card.
	virtual void test() = 0;
