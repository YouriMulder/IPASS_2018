...
bool MFRC522::getCardUIDSimple(uint8_t UID[5]) {
	if(isCardInRange()) {
		return (getCardUID(UID) == OkStatus);
	}
	return false;
}
...
void MFRC522::printUID() {
	uint8_t UID[5] = {0};
	waitForCardUID(UID);
	hwlib::cout << static_cast<unsigned>(UID[0])
	            << ", " << static_cast<unsigned>(UID[1])
	            << ", " << static_cast<unsigned>(UID[2])
	            << ", " << static_cast<unsigned>(UID[3])
	            << ", " << static_cast<unsigned>(UID[4]) << "\n";
}

void MFRC522::printTest(bool condition, const char testCaseMessage[]) {
...
void MFRC522::test() {
	...
	// self test
	printTest(selfTest(), "self test");

	// Self test uses soft reset so need to init
	initialize();

	// check card
	hwlib::cout << "Waiting for card! Place a card in the field of the chip\n";
	for(int i = 0; i < 100; i++) {
		if(isCardInRangeCheck()) {
			break;
		}
		hwlib::wait_ms(50);
	}
	...
	uint8_t UID[5] = {0};
	UID[4] = 20;
	printTest(getCardUIDSimple(UID), "card in range");
	printTest(checkUID4BCC(UID), "card UID valid");
