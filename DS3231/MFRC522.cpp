...
void MFRC522::waitTillStarted() {
    while(readRegister(CommandReg) & (1 << 4)) {hwlib::cout << "test";}
}
...
void MFRC522::clearFIFOBuffer() {
    // clears the FIFO read and write pointer, ErrorReg and BufferOvfl
    writeRegister(FIFOLevelReg, 0x80);

    uint8_t FIFOnewBytes[25] = {0};
    writeRegister(FIFODataReg, FIFOnewBytes, 25);
...
bool MFRC522::selfTest() {
    ...
    for(uint8_t i = 0; i < FIFOAmountOfBytes; i++) {
        hwlib::cout << hwlib::hex << (unsigned)readRegister(FIFODataReg) << "\n";
    }
    hwlib::cout << "-------\n\n";
